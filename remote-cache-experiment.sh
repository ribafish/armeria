...
do
    ...
    elif [ "$cache" == 'kotlin-dsl' ]
    then
        disabledCacheArgs='-Dorg.gradle.internal.kotlin-script-caching-disabled'
    fi
    for args in "-Dscan.tag.baseline-$cache" "-Dscan.tag.disabled-cache-$cache $disabledCacheArgs"
    do
        echo "------------------------------------------------------------"
        ...
        ./gradlew $task -g $homeDir --no-configuration-cache -Ddevelocity.deprecation.muteWarnings=true -Dscan.uploadInBackground=false -Dgradle.enterprise.url=https://ge.solutions-team.gradle.com/ $args
